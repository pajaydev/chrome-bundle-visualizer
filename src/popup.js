(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('fs'), require('path'), require('child_process'), require('os')) :
        typeof define === 'function' && define.amd ? define(['fs', 'path', 'child_process', 'os'], factory) :
            (global = global || self, factory(global.fs, global.path, global.child_process, global.os));
}(this, (function (fs, path, child_process, os) {
    'use strict';
    ...
    pp$1.parseLabeledStatement = function (node, maybeName, expr) {
        ...
        if (node.body.type === "ClassDeclaration" ||
            ...
            node.body.type === "FunctionDeclaration" && (this.strict || node.body.generator || node.body.async)) { this.raiseRecoverable(node.body.start, "Invalid labeled declaration"); }
        this.labels.pop();
        node.label = expr;
    ...
    var walk = createCommonjsModule(function (module, exports) {
        (function (global, factory) {
            ...
            function makeTest(test) {
                if (typeof test === "string") { return function (type) { return type === test; } }
                else if (!test) { return function () { return true; } }
                else { return test }
            }
                ...
                try {
                    (function c(node, st, override) {
                        ...
                        if ((start == null || node.start <= start) &&
                            (end == null || node.end >= end)) { baseVisitor[type](node, st, c); }
                        if ((start == null || node.start === start) &&
    ...
    class Stats {
        ...
        setNonLasso(nonLasso) {
            this.nonLasso = nonLasso;
        }

        getNonLasso() {
            return this.nonLasso;
        }
    ...
    const getArguments$1 = utils.getArguments;
    const isMemberExpression$1 = utils.isMemberExpression;
    const extractLiteralFromInstalled$1 = utils.extractLiteralFromInstalled;
    const extractLiteralFromDef$1 = utils.extractLiteralFromDef;
    const extractLiteralFromMain$1 = utils.extractLiteralFromMain;
    const isValidFunctionExpression$1 = utils.isValidFunctionExpression;
    const isFunctionExpression$1 = utils.isFunctionExpression;

    function parseLassoBundle(fileContent) {
        ...
        const ast = acorn_es.parse(fileContent, {
            sourceType: 'script'
        });
        ...
        walk.recursive(
            ...
            walkState,
            {
                CallExpression(node, state, c) {
                    const stats$1 = new stats();
                    if (node.start && node.end) {
                        stats$1.setSize(node.end - node.start);
                    } else {
                        stats$1.setSize(0);
                    }
                    if (isFunctionExpression$1(node)) {
                        stats$1.setPackageName("module.js");
                        stats$1.setFileName("module.js");
                        stats$1.setPath("/module.js");
                    }
                    if (isMemberExpression$1(node)) {
                        let memberNode = node.callee;
                        if (memberNode.property && memberNode.property.type === "Identifier") {
                            stats$1.setType(memberNode.property.name);
                        }
                    }
                    const args = getArguments$1(node);
                    if (args.length > 0) {
                        extractLiterals(stats$1, args);
                    }
                    if (stats$1.getType() === "def") {
                        extractContent(fileContent, stats$1, args[1]);
                    }
                    walkState.push(stats$1);
                },
            });
        // const manifestPath = process.cwd();
        // fs.writeFileSync(manifestPath + '/lasso-stats.json', JSON.stringify(walkState, null, 2));
        // return { walkState };
        return JSON.stringify(walkState, null, 2);
    }
    ...
    function extractLiterals(stats, args) {
        if (stats.getType() != null && (stats.getType() === "installed" || stats.getType() === "builtin")) {
            extractLiteralFromInstalled$1(stats, args);
        }

        if (stats.getType() != null && stats.getType() === "def") {
            extractLiteralFromDef$1(stats, args[0]);
        }

        if (stats.getType() != null && (stats.getType() === "main" || stats.getType() === "remap")) {
            extractLiteralFromMain$1(stats, args[0]);
        }
    }
    ...
    function extractContent(fileContent, stats, node) {
        if (isValidFunctionExpression$1(node)) {
            let body = node.body;
    ...
    var lassoUnpack = parseLassoBundle;
    ...
    function bundleAnalyzer(readJSON) {
        //readJSON = JSON.parse(readJSON);
        if (readJSON.length > 0) {
            readJSON.shift();
        } const tree$1 = new tree('/');
        tree$1.setEnableColors(true);
        readJSON.forEach((source) => {
            tree$1.createNode(source, tree$1, tree$1.isColorEnabled());
        });
        tree$1.createTile(tree$1, tree$1.data['$area']);
        var event = new CustomEvent('lasso-build', { detail: tree$1 });

        // Dispatch the event
        window.dispatchEvent(event);
    }

    var _src = bundleAnalyzer;
    ...
    scriptsPromise.then(results => {
        let lassoContent = results.reduce((sumContent, eachContent) => {
            if (eachContent.indexOf("$_mod.def") > -1) return sumContent + eachContent;
            return sumContent;
        });
        ...
        let nonLassoJSON = results.reduce((sumJSON, eachContent) => {
            if (eachContent.indexOf("$_mod.def") === -1) {
                ...
                if (funcName) {
                    ...
                    stats.setFileName(funcName);
                    stats.setPath(`/${funcName}`);
                    stats.setSize(eachContent.length);
                    stats.setNonLasso("true");
                    sumJSON.push(stats);
                };
            }
            return sumJSON;
        }, []);
        let lassoJSON = lassoUnpack(lassoContent);
        let mergedJSON = [...JSON.parse(lassoJSON), ...nonLassoJSON];
        _src(mergedJSON);
    });

    function getFunctionName(script) {
        return script.match(/([a-zA-Z_{1}][a-zA-Z0-9_]+)(?=\()/g)[0];
    }
